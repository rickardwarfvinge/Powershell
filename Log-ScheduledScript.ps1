Function Log-ScheduledScript () {
    ...
    .EXAMPLE 
    Log-ScheduledScript -EventLogName 'Powershell Script Log' -EventSourceName Scripts -EventIDInfo 100 -EventIDError 101 -EventTimeType minutes -EventFrequencyInterval 5
    ...
    
    {
        ...
        {$EventTimeType -eq 'seconds' -and $TimeDiff.TotalSeconds -gt $EventFrequencyInterval} {Write-EventLog -LogName $EventLogName -Source $EventSourceName -EventId $EventIDError -EntryType Error -Message "$LogMessage second(s)."}
        {$EventTimeType -eq 'minutes' -and $TimeDiff.TotalMinutes -gt $EventFrequencyInterval} {Write-EventLog -LogName $EventLogName -Source $EventSourceName -EventId $EventIDError -EntryType Error -Message "$LogMessage minute(s)."}
        {$EventTimeType -eq 'hours' -and $TimeDiff.TotalHours -gt $EventFrequencyInterval} {Write-EventLog -LogName $EventLogName -Source $EventSourceName -EventId $EventIDError -EntryType Error -Message "$LogMessage hour(s)."}
